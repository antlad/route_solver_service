...
function way_function (way, result)
  ...
  -- We don't route over route with maxweight
  local maxweight = parse_maxweight(way:get_value_by_key("maxweight"))
  if 0 < maxweight then
     if vehicle_max_weight >= maxweight then
  return
     end
  end

  -- We don't route over route with maxheight
  local maxheight = parse_maxheight(way:get_value_by_key("maxheight"))
  if 0 < maxheight then
     if vehicle_max_height >= maxheight then
  return
     end
  end
